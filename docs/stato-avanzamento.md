...
**Blocco 3-bis — Secondo template (studio professionale): chiuso. Il checkpoint è passato.** [`orecchioni/template-studio-professionale`](https://github.com/orecchioni/template-studio-professionale), terzo repo del progetto. Verticale scelto come **test più duro** fra quelli previsti: niente galleria naturale, niente testimonianze pubbliche tipiche, niente prezzi esposti (`prezzo_nota: "su preventivo"`), orari da ufficio invece che da locale.

**Cosa ha retto senza modifiche:** otto sezioni su nove, `Layout.astro`, tutti i componenti `ui/`, `lib/content.ts`, `lib/tema.ts`, `lib/informativa.ts`, `lib/markdown-minimo.ts`, tutte le `pages/` e i due script di controllo — copiati e mai toccati. **Il componente form e la struttura dello schema non sono stati modificati**, che era il criterio vero del checkpoint.

**Cosa è stato adattato nel template (non nello schema):** il dizionario dei tag alimentari in `Servizi.astro` (D5 è specifico dell'alimentare: il campo resta, in questo verticale è sempre vuoto) e `@type` dei dati strutturati (`Restaurant` → `ProfessionalService`, una riga, scelta del template come la mappa statica).

**Cosa ha costretto a toccare lo schema condiviso — l'unica cosa, e registrata come D45:** `azienda.social` aveva `tripadvisor` come chiave fissa e nessun `linkedin`. Per uno studio legale è esattamente al contrario. Aggiunto il campo, propagato a `content.example.json`, al `content.json` del ristorante e al `Footer.astro` di entrambi i template. **Il primo build ha mostrato una mezza modifica:** campo nello schema, footer vuoto — il componente non lo rendeva. Corretto e riverificato in entrambi i repo.

**Verifiche fatte, non assunte:** build pulita al primo colpo dopo l'adattamento; i due casi negativi (campo obbligatorio mancante, `informativa_versione` inesistente) falliscono con errore leggibile; con `gallery` e `testimonianze` disattivate l'output contiene solo le sei sezioni attese e il layout regge; nessuna richiesta a terze parti; screenshot mobile a 390px veri via CDP con `innerWidth === scrollWidth`, nessun overflow; `template-ristorante` ricostruito dopo ogni modifica allo schema condiviso, sempre verde.

**Conclusione del checkpoint:** lo scheletro condiviso regge su un verticale lontano dal ristorante. Il costo del secondo template è stato nettamente inferiore al primo — il grosso del lavoro è stato `content.json` e un adattamento da due file. Si può procedere con i restanti verticali (Blocco 3-ter…).
...
| 3-bis — Secondo template | **chiuso** | `template-studio-professionale`: checkpoint passato. Form e struttura schema invariati; un solo campo aggiunto (D45) |
| 3-ter… — Restanti template | **pronto a partire** | Prossimi: parrucchiere, studio dentistico. Si costruiscono col disegno placeholder (D46): il design verticale per categoria è un lavoro a sé, dovuto prima del Blocco 4 |
...
| 2026-08-14 | D46 | Registrato che il design è **verticale per categoria**, non una formula unica: un dentista non può avere il linguaggio visivo di uno studio legale. Si procede col placeholder per costruire il sistema; il design vero è un lavoro a sé, dovuto prima del Blocco 4 (il cui criterio è «senza vergognarsi di mostrarla»). Costa meno del previsto: `palette_N` e `font_N` sono identificatori, non valori — ogni template li mappa nel proprio `tema.ts`, quindi il design verticale non tocca il pacchetto condiviso. Unico punto che stona: `tema.tono` è globale e non letto da nessun template — D39 sale a decisione dovuta prima del Blocco 4. |
| 2026-08-14 | Blocco 3-bis | Costruito `template-studio-professionale` (terzo repo). Checkpoint dello scheletro condiviso **passato**: componente form e struttura dello schema invariati, otto sezioni su nove copiate senza modifiche. Adattati nel template il dizionario dei tag alimentari e `@type` dei dati strutturati. Un solo campo aggiunto allo schema condiviso (`azienda.social.linkedin`, D45) — scoperto che il campo da solo non basta: il `Footer.astro` non lo rendeva, bug visto al primo build e corretto in entrambi i template. Verificati i due casi negativi, il layout con due sezioni disattivate, l'assenza di terze parti e l'assenza di overflow a 390px veri. |
