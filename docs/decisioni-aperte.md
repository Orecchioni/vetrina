...
| D39 | Valori di `tema.tono` | `familiare`, `professionale`, `contemporaneo` | ⚠️ **Da chiudere prima del Blocco 4, insieme a D46.** Due indizi convergenti: §6.2 lo voleva «2-3 per categoria» e l'enum è globale; **nessun template lo legge** (verificato al Blocco 3-bis: zero occorrenze in `src/` di entrambi). Con D46 la scelta è secca — o per-categoria come `palette`, o si toglie |
...
| D46 | Il design è **verticale per categoria**, non una formula unica declinata | §6.2, D38, D39, D44 | **Ogni categoria ha il proprio linguaggio visivo.** Un dentista non può avere il design di uno studio legale, un parrucchiere ancora meno: standardizzare una formula per tutti risparmierebbe lavoro a costo della credibilità davanti al cliente finale. **Adesso si procede con il disegno placeholder attuale** — serve a costruire il sistema, non a venderlo, e i template esistenti sono dichiarati tali nei rispettivi README. **Poi serve un lavoro di design vero, categoria per categoria.** <br><br>**Quanto costa: molto meno di quanto sembri, perché lo schema lo supporta già.** `palette_1`…`palette_4` e `font_1`…`font_3` sono *identificatori*, non valori: i colori e i font veri vivono in `src/lib/tema.ts` di ciascun template. `palette_1` può essere terracotta nel ristorante e azzurro clinico nel dentista — stessa chiave nel `content.json`, linguaggi visivi diversi, **zero modifiche al pacchetto condiviso**. Il lavoro verticale è possibile per costruzione (conseguenza non pianificata ma utile di D1: repo autonomi, CSS non condiviso). <br><br>**Cosa invece stona, e va deciso:** `tema.tono` (D39) è un enum **globale** (`familiare`, `professionale`, `contemporaneo`) mentre §6.2 lo voleva «2-3 per categoria» — ed è l'unico campo dello schema che **nessun template legge**. Con D46 il sospetto diventa concreto: o diventa per-categoria come `palette`, o si toglie prima che si consolidi in dieci template e in qualche `content.json` consegnato. <br><br>**Scadenza raccomandata: prima del Blocco 4.** Il criterio di quel blocco è «si apre dal telefono senza vergognarsi di mostrarla»: è la prima cosa mostrata a qualcuno, e un placeholder non lo supera. | **decisa 2026-08-14 · design per categoria dovuto prima del Blocco 4** |
